...
void print_result_find(int64_t time, int mem, int comp)
{
    printf("Время для поиска (в тиках): %ld", time);
    printf("\nПамяти зайдествовано: %d", mem);
    printf("\nСравнений для поиска: %d\n\n", comp);
}
...
int find_all(node_t *avl, node_t *ddp, arr_t *arr, hash_t *hash_table)
{
    printf("\n\nВведите элемент для поиска:  ");

    int num = 0, data, flag = 1;
    ...
    if (flag)
    {
        printf("Ошибка: нет такого числа");
        return 1;
    }
    ...
    int64_t start = 0, end = 0;
    start = tick();
    find_tree(ddp, data, &num);
    end = tick();
    int size = arr->len * sizeof(node_t);
    print_result_find((end - start), size, num);
    
    puts("\n\nЗамеры для АВЛ");
    num = 0, start = 0, end = 0;
    start = tick();
    find_tree(avl, data, &num);
    end = tick();
    size = arr->len * sizeof(node_t);
    print_result_find((end - start), size, num);
    
    puts("\n\nЗамеры для ХЕШ-таблицы");
    num = 1, start = 0, end = 0;
    start = tick();
    find_hash(hash_table, data);
    end = tick();
    size = hash_table->size * 2 * sizeof(int) + sizeof(int);
    print_result_find((end - start), size, num);

...
void find_hash(hash_t *hash_table, int data)
{
    int index = key_hash(data, hash_table->size);
    
    if (hash_table->data[index] == data)
        return;

    //for (int i = index; i < hash_table->size; i++)
    //{
    //    if (hash_table->data[i] == data)
    //        return;
    //}
    //printf("ERROR");
}
